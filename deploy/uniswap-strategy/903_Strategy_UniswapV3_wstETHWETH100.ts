...
const func: DeployFunction = async function(hre: HardhatRuntimeEnvironment) {
  ...
  const { deployer, CONVERTER_ADDRESS, UNISWAPV3_WSTETH_WETH_100, SPLITTER_WETH_ADDRESS } = await getNamedAccounts();

  if (await isContractExist(hre, 'Strategy_UniswapV3ConverterStrategy_wstETHWETH100')) {
    return;
  }
  ...
  const proxyDeployResult = await deployments.deploy('Strategy_UniswapV3ConverterStrategy_wstETHWETH100', {
  ...
  await deployments.execute(
    'Strategy_UniswapV3ConverterStrategy_wstETHWETH100',
    {
      from: deployer,
      log: true,
      ...(await txParams(hre, ethers.provider)),
