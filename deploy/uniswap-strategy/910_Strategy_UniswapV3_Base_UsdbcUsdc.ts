...
const func: DeployFunction = async function(hre: HardhatRuntimeEnvironment) {
  ...
  const { deployer, CONVERTER_ADDRESS, UNISWAPV3_BASE_USDC_USDbC_100, SPLITTER_USDbC_ADDRESS } = await getNamedAccounts();

  if (await isContractExist(hre, 'Strategy_UniswapV3ConverterStrategy_Base_UsdbcUsdc')) {
    return;
  }
  ...
  const proxyDeployResult = await deployments.deploy('Strategy_UniswapV3ConverterStrategy_Base_UsdbcUsdc', {
...
export default func;
func.tags = ['Strategy_UniswapV3ConverterStrategy_Base_UsdbcUsdc'];
