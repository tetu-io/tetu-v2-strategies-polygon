...
async function quoteOneInch(fromTokenAddress: string, toTokenAddress: string, amount: string, fromAddress: string, chainId: number, protocols?: string): Promise<IAggQuote|undefined> {
  const params = {
    fromTokenAddress,
    toTokenAddress,
    amount,
...
async function quoteOpenOcean(inTokenAddress: string, outTokenAddress: string, amount: string, account: string, chainId: number): Promise<IAggQuote|undefined> {
  const params = {
  ...
  const url = `https://open-api.openocean.finance/v3/${openOceanChains[chainId.toString()]}/swap_quote?${(new URLSearchParams(JSON.parse(JSON.stringify(params)))).toString()}`
...
Web3Function.onRun(async (context: Web3FunctionContext) => {
  ...
  const agg = userArgs.agg as string
    ...
    if (aggQuote) {
      return {
        ...
        callData: [
          {
            to: strategyAddress,
            data: strategy.interface.encodeFunctionData("rebalanceSwapByAgg", [quote[0], quote[1], aggQuote.to, aggQuote.data]),
          },
        ],
      };
    }
