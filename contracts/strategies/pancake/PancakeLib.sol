...
import "../../integrations/algebra/IPancakePool.sol";
...
library PancakeLib {
  ...
  function addLiquidityPreview(address pool_, int24 lowerTick_, int24 upperTick_, uint amount0Desired_, uint amount1Desired_) external view returns (uint amount0Consumed, uint amount1Consumed, uint128 liquidityOut) {
    IPancakePool pool = IPancakePool(pool_);
  ...
  function getFees(IPancakePool pool, INonfungiblePositionManager nft, uint tokenId) public view returns (uint fee0, uint fee1) {
