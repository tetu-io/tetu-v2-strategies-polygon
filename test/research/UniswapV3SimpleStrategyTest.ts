...
import {
  ISwapper,
  ISwapper__factory,
  UniswapV3SimpleStrategy, UniswapV3SimpleStrategy__factory
} from "../../typechain";
...
describe('UniswapV3SimpleStrategyTests', function() {
  let snapshotBefore: string;
  let snapshot: string;
  let signer: SignerWithAddress;
  let signer2: SignerWithAddress;
  let strategy: UniswapV3SimpleStrategy;
  ...
  before(async function() {
    ...
    // const range = 375;
    // +-0.5% price - rebalance
    const rebalanceRange = 20;

    strategy = await DeployerUtils.deployContract(signer, 'UniswapV3SimpleStrategy', poolAddress, range, rebalanceRange) as UniswapV3SimpleStrategy
  ...
  describe('UniswapV3 research strategy tests', function() {
    ...
    it('Rebalance cost', async () => {
      ...
      const priceMoveAmount = parseUnits('25000', 6)
      const priceMoveIterations = 20
      const priceMoveAmount2 = parseUnits('12500', 6)
      const priceMoveIterations2 = 20
      let rebalances = 0
      await TokenUtils.getToken(tokenIn, signer.address, depositAmount);
      await TokenUtils.approve(tokenIn, signer, strategy.address, depositAmount.toString())
      ...
      for(let i = 0; i < priceMoveIterations; i++) {
        await movePriceUp(signer, strategy.address, priceMoveAmount)
      ...
      const price2 = await swapper.getPrice(await strategy.pool(), MaticAddresses.WMATIC_TOKEN, MaticAddresses.ZERO_ADDRESS, 0);

      for(let i = 0; i < priceMoveIterations2; i++) {
        await movePriceUp(signer, strategy.address, priceMoveAmount2)
        if (await strategy.needRebalance()) {
          rebalances++;
          await strategy.rebalance()
        }
      ...
      console.log('RESULTS')
      console.log(`TickRange: ${await strategy.tickRange()}`)
      console.log(`RebalanceThreshold: ${await strategy.rebalanceTickRange()}`)
      console.log(`Price moving: ${priceStart} -> ${priceMax} -> ${priceEnd}`)
      console.log(`Rebalances: ${rebalances}`)
      console.log(`Total loss after rebalances: ${loss} (${formatUnits(loss.mul(10**10).div(estimatedBefore).div(10**4), 4)}%)`)
      console.log(`-------------------------------------`)
    })
...
async function movePriceUp(signer: SignerWithAddress, strategyAddress: string, amount: BigNumber) {
  const strategy = UniswapV3SimpleStrategy__factory.connect(strategyAddress, signer) as UniswapV3SimpleStrategy
