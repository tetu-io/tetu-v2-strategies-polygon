...
describe.skip('UmiswapV3 converter strategy backtester', function() {
  ...
  before(async function() {
    ...
    usdcWeth005PoolLiquiditySnapshot = await UniswapV3Utils.getPoolLiquiditySnapshot(
      getAddress(MaticAddresses.UNISWAPV3_USDC_WETH_500),
      backtestStartBlock,
      pool500LiquiditySnapshotSurroundingTickSpacings,
    );
    wmaticUsdc005PoolLiquiditySnapshot = await UniswapV3Utils.getPoolLiquiditySnapshot(
      getAddress(MaticAddresses.UNISWAPV3_WMATIC_USDC_500),
      backtestStartBlock,
      pool500LiquiditySnapshotSurroundingTickSpacings,
    );
    usdcDai001PoolLiquiditySnapshot = await UniswapV3Utils.getPoolLiquiditySnapshot(
      getAddress(MaticAddresses.UNISWAPV3_USDC_DAI_100),
      backtestStartBlock,
      pool100LiquiditySnapshotSurroundingTickSpacings,
    );
    usdcUsdt001PoolLiquiditySnapshot = await UniswapV3Utils.getPoolLiquiditySnapshot(
      getAddress(MaticAddresses.UNISWAPV3_USDC_USDT_100),
      backtestStartBlock,
      pool100LiquiditySnapshotSurroundingTickSpacings,
    );
    ...
    wmaticUsdc005Pool = UniswapV3Pool__factory.connect(await uniswapV3Factory.getPool(
      WMATIC.address,
      USDC.address,
      500,
    ), signer);
    await (await uniswapV3Factory.createPool(USDC.address, WETH.address, 500)).wait();
    usdcWeth005Pool = UniswapV3Pool__factory.connect(
      await uniswapV3Factory.getPool(USDC.address, WETH.address, 500),
      signer,
    );
    await (await uniswapV3Factory.createPool(USDC.address, DAI.address, 100)).wait();
    usdcDai001Pool = UniswapV3Pool__factory.connect(
      await uniswapV3Factory.getPool(USDC.address, DAI.address, 100),
      signer,
    );
    await (await uniswapV3Factory.createPool(USDC.address, USDT.address, 100)).wait();
    usdcUsdt001Pool = UniswapV3Pool__factory.connect(
      await uniswapV3Factory.getPool(USDC.address, USDT.address, 100),
      signer,
    );
    ...
    cUSDC = await DeployerUtils.deployContract(
      signer,
      'CErc20Immutable',
      USDC.address,
      comptroller.address,
    ...
    cWETH = await DeployerUtils.deployContract(
      signer,
      'CErc20Immutable',
      WETH.address,
      comptroller.address,
    ...
    cWMATIC = await DeployerUtils.deployContract(
      signer,
      'CErc20Immutable',
      WMATIC.address,
      comptroller.address,
    ...
    cDAI = await DeployerUtils.deployContract(
      signer,
      'CErc20Immutable',
      DAI.address,
      comptroller.address,
    ...
    cUSDT = await DeployerUtils.deployContract(
      signer,
      'CErc20Immutable',
      USDT.address,
      comptroller.address,
  ...
  it('Env test', async function() {
    if (runNotBacktestingTests) {
      ...
      expect(await compPriceOracleImitator.getUnderlyingPrice(cWETH.address))
        .eq(liquidatorWethPrice.mul(parseUnits('1', 12)));
      expect(await compPriceOracleImitator.getUnderlyingPrice(cWMATIC.address))
        .eq(liquidatorWmaticPrice.mul(parseUnits('1', 12)));
      expect(await compPriceOracleImitator.getUnderlyingPrice(cDAI.address))
        .eq(liquidatorDaiPrice.mul(parseUnits('1', 12)));
      expect(await compPriceOracleImitator.getUnderlyingPrice(cUSDT.address))
        .eq(liquidatorUsdtPrice.mul(parseUnits('1', 24)));
      expect(await priceOracleImitator.getAssetPrice(WETH.address)).eq(liquidatorWethPrice.mul(parseUnits('1', 12)));
      expect(await priceOracleImitator.getAssetPrice(WMATIC.address))
        .eq(liquidatorWmaticPrice.mul(parseUnits('1', 12)));
  ...
  it('Rebalance on stable pool with fuse test', async function() {
    if (runNotBacktestingTests) {
      ...
      const pool = UniswapV3Pool__factory.connect(
        await uniswapV3Factory.getPool(DAI.address, USDT.address, 100),
        signer,
      );
      // 1 USDT for 1 DAI
      ...
      const liquidityPreview = await uniswapV3Helper.addLiquidityPreview(
        pool.address,
        liquidityTickLower,
        liquidityTickUpper,
        parseUnits('100000'),
      ...
      console.log(`Total assets loss: ${formatUnits(
        assetsAfter.sub(assetsBefore).mul(1e15).div(assetsBefore).div(1e8),
        5,
      )}%`);
    }
  });
  ...
  it('Full debt rebalance on volatile pool test', async function() {
    if (runNotBacktestingTests) {
      ...
      const pool = UniswapV3Pool__factory.connect(
        await uniswapV3Factory.getPool(WMATIC.address, USDT.address, 500),
        signer,
      );
