  ...
  platforms.forEach(function (platformType: PlatformsType) {
    ...
    describe(`${platformType}`, function () {
          ...
          describe("Move prices down", () => {
            ...
            before(async function () {
              ...
              ptr = await PairWithdrawByAggUtils.prepareWithdrawTest(signer, signer2, builderResults, {
                movePricesUp: false,
                pathTag: "#down1",
                countRebalances: platformType === PLATFORM_ALGEBRA ? 1 : 2
              });
            });
            after(async function () {
              await TimeUtils.rollback(snapshotLevel0);
      ...
      describe('scb-792: Unfold debts using single iteration, MockSwapper changes prices', function () {
        ...
        TEST_SETUPS[platformType].forEach(function (testSetup: ITestSetup) {
          ...
          async function prepareStrategy() {
            const converterStrategyBase = await ConverterStrategyBase__factory.connect(builderResults.strategy.address, signer);
            const platformVoter = await DeployerUtilsLocal.impersonate(
                await IController__factory.connect(await converterStrategyBase.controller(), signer).platformVoter()
            );
            await ConverterStrategyBase__factory.connect(builderResults.strategy.address, platformVoter).setCompoundRatio(0);

            // provide $1000 of insurance to compensate possible price decreasing
            await PairBasedStrategyPrepareStateUtils.prepareInsurance(builderResults, "1000");
          }

          describe(`${platformType}-${testSetup.increaseOutput ? "amountOutUpper" : "amountOutLower"}`, () => {
            describe("Liquidator, entry to pool at the end", () => {
              let snapshot: string;
              before(async function () {
                snapshot = await TimeUtils.snapshot();
      ...
      describe('rebalanceNoSwaps', function () {
        ...
        TEST_SETUPS[platformType].forEach(function (testSetup: ITestSetup) {
          describe(`${platformType}-${testSetup.priceUp ? "up" : "down"}`, () => {
            let snapshot: string;
            ...
            async function makeRebalance(): Promise<IRebalanceResults> {
              ...
              let eventsSet = await CaptureEvents.makeDeposit(b.vault.connect(signer), parseUnits(testSetup.depositAmount, 6), platformType);
