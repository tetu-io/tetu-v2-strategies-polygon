...
describe.skip('AlgebraConverterStrategyAggRebalanceTest', function() {
  ...
  describe('Algebra strategy rebalance by agg tests', function() {
    it('Rebalance', async() => {
      const s = strategy
      const defaultState = await s.getDefaultState();
      const addrTokenA = defaultState.addr[IDX_DEFAULT_STATE_TOKEN_A];
      const addrTokenB = defaultState.addr[IDX_DEFAULT_STATE_TOKEN_B];
      const addrPool = defaultState.addr[IDX_DEFAULT_STATE_POOL];
      ...
      const params = {
        fromTokenAddress: quote[0] ? addrTokenA : addrTokenB,
        toTokenAddress: quote[0] ? addrTokenB : addrTokenA,
