...
import {
  ...
  IStrategyV2, PancakeConverterStrategy, PancakeConverterStrategy__factory, PancakeLib,
  TetuVaultV2,
} from '../../../../typechain';
...
describe('PancakeConverterStrategyUniversalBaseTest', async () => {
  ...
  const targets: [string, string, number, number][] = [
    [BaseAddresses.USDbC_TOKEN, BaseAddresses.PANCAKE_POOL_USDC_USDbC_LP_100, 0, 0],
    [BaseAddresses.USDbC_TOKEN, BaseAddresses.PANCAKE_POOL_DAI_USDbC_LP_100, 0, 0],
  ]
  ...
  targets.forEach(t => {
    ...
    const deployer = async(signer: SignerWithAddress) => UniversalTestUtils.makeStrategyDeployer(
      ...
      async(strategyProxy: string, signerOrProvider: Signer | Provider, splitterAddress: string) => {
        ...
        await strategy.init(core.controller, splitterAddress, tetuConverterAddress, t[1], t[2], t[3], [0, 0, Misc.MAX_UINT, 0], BaseAddresses.PANCAKE_MASTER_CHEF_V3);
        const mainAssetSymbol = await IERC20Metadata__factory.connect(asset, signer).symbol()
        statesParams[t[1]] = {
          mainAssetSymbol,
        }
        const state = await PackedData.getDefaultState(strategy);
        const profitHolder = await DeployerUtils.deployContract(signer, 'StrategyProfitHolder', strategy.address, [state.tokenA, state.tokenB, BaseAddresses.PANCAKE_SWAP_TOKEN])
