...
inputFileSources.forEach(
  (s) => {
    s.value.name      = s.parentProperty;
    files[s.value.id] = s.value;
  },
);
...
const allContracts = JSONPath({ json: buildInfo, path: `$.output.contracts` })[0]
...
function enumPathToLeaf (root, childEnum, childPredicate, mutator) {
  childEnum(root).forEach((c) => {
    if (childPredicate(c)) {
      enumPathToLeaf(c, childEnum, childPredicate, mutator);
    }
  });
  mutator(root);
}
...
//console.log('create sourceMapEntries');
...
sourceMapEntries.forEach((e, i) => {
  ...
  enumPathToLeaf(
    files[entry[2]].ast,
    (n) => {
      return n.statements || n.nodes || (n.body ? [n.body] : undefined) || [];
    },
    (c) => {
    ...
    (n) => {
      if (n.byteSize === undefined) {
        n.byteSize = 0;
      }
      n.byteSize += opcodeByteSize;
      if (n.opCount === undefined) {
        n.opCount = 0;
      }
      n.opCount++;
    },
...
//console.log('sourceMapEntries handled');
...
files.forEach((f) => {
  try {
    //console.log('files', f.ast.nodes);
    if (f.ast) {
      const funcNodes = JSONPath({
        json: f.ast,
        path: `$.nodes..*[?(@.nodeType === "FunctionDefinition" || @.nodeType === "ModifierDefinition" || @.nodeType === "YulFunctionDefinition")]`,
      });
...
console.log('funcDefs handled', funcDefs.length);
...
if (args.outputType === "listing") {
  funcDefs.forEach((fd) => {
    if(fd.byteSize) {
      console.log(`${fd.byteSize ? fd.byteSize : "?"}\t\t${files[fd.src.split(':')[2]].name}:${fd.name != ""
        ? fd.name
        : "(constructor)"}`);
    }
  });
...
if (args.outputType == "script") {
  ...
  entryFuncInfo.forEach((e, i) => {
    console.log(`EvmOp entry<bgcolor = ${nameToColorHexString("bg" + e.name)}, fgcolor = ${nameToColorHexString("fg" + e.name)}, comment = "${files[e.src.split(
      ':')[2]].name}:${e.name != "" ? e.name : "(constructor)"}", optimize = false>;`)
  });
}
