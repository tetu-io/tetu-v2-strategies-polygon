...
import {
  ControllerV2__factory, IStrategyV2__factory,
  ...
  for (const vault of vaults) {
    ...
    for (const strategyAddress of strategies) {
      if (await NSRUtils.isStrategyEligibleForNSR(strategyAddress)) {
        ...
        if (
          strategyConfigs[strategyAddress].lockedPercentForDelayedRebalance !== config.lockedPercentForDelayedRebalance.toNumber()
          || strategyConfigs[strategyAddress].lockedPercentForForcedRebalance !== config.lockedPercentForForcedRebalance.toNumber()
          || strategyConfigs[strategyAddress].rebalanceDebtDelay !== config.rebalanceDebtDelay.toNumber()
        ) {
          ...
          await RunHelper.runAndWaitAndSpeedUp(
            ...
            () => configContract.setConfig(
              strategyAddress,
              strategyConfigs[strategyAddress].lockedPercentForDelayedRebalance,
              strategyConfigs[strategyAddress].lockedPercentForForcedRebalance,
              strategyConfigs[strategyAddress].rebalanceDebtDelay,
              { ...tp }),
            true,
            true,
